...
# Function for grok API call
def grok_inventory(prompt, dim, aux):
    dataset = getItems(ITEMPATH, TEST_TYPE)
    batch_size = 1
    result = []
    
    for i in tqdm(range(0, len(dataset), batch_size), desc="grok Progress"):
    ...
    filename = f"grok_MPI_{dim}_{aux}.pickle"
