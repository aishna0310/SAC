...
def get_batch_response_grok(prompt):
    messages=[
...
for index, row in traits_df.iterrows():
    ...
    for i, intensity_row in intensity_df.iterrows():
        ...
        grok_response = get_batch_response_grok(batch_prompt)
        
        try:
            grok_responses = grok_response.split("\n")
            print(f"grok Responses: {grok_responses}")  # Debugging print
            
            if len(grok_responses) < 2:
                raise IndexError("Expected at least two lines in grok response.")

            grok_score_1 = float(grok_responses[0].split(":")[-1].strip())
            grok_score_2 = float(grok_responses[1].split(":")[-1].strip())

            if trait not in results_grok:
                results_grok[trait] = []
            results_grok[trait].extend([grok_score_1, grok_score_2])
...
grok_stats = calculate_stats(results_grok)

# Print or save results for comparison
for trait in traits_df['Personality'].unique():
    print(f"\nTrait: {trait}")
    print(f"grok - Mean: {grok_stats[trait]['mean']}, Variance: {grok_stats[trait]['variance']}")
