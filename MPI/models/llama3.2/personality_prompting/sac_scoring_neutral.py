...
def get_batch_response_llama(prompt):
    messages=[
...
for index, row in traits_df.iterrows():
    ...
    for i, intensity_row in intensity_df.iterrows():
        ...
        llama_response = get_batch_response_llama(batch_prompt)
        
        try:
            llama_responses = llama_response.split("\n")
            print(f"llama Responses: {llama_responses}")  # Debugging print
            
            if len(llama_responses) < 2:
                raise IndexError("Expected at least two lines in gemini response.")

            llama_score_1 = float(llama_responses[0].split(":")[-1].strip())
            llama_score_2 = float(llama_responses[1].split(":")[-1].strip())

            if trait not in results_llama:
                results_llama[trait] = []
            results_llama[trait].extend([llama_score_1, llama_score_2])
...
llama_stats = calculate_stats(results_llama)

# Print or save results for comparison
for trait in traits_df['Personality'].unique():
    print(f"\nTrait: {trait}")
    print(f"llama - Mean: {llama_stats[trait]['mean']}, Variance: {llama_stats[trait]['variance']}")
